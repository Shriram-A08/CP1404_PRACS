"""
CP1404/CP5632 Practical - Miles to Kilometres Converter
...
class MilesConverterApp(App):
    ...
    def handle_increment(self, text, change):
        """Handle up/down button press to change the miles value."""
        print("handle increment")
        miles = self.convert_to_number(text) + change
        self.root.ids.input_miles.text = str(miles)
        # on_text event will call handle_calculate
    ...
    def convert_to_number(text):
        """Convert text to a float or return 0.0 if invalid."""
        try:
            return float(text)
        except ValueError:
            return 0.0
